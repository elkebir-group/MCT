...
void MctSolverIlpCallbackUser::init()
{
  const int m = _b.size();
...
void MctSolverIlpCallbackUser::writeDOT(std::ostream& out) const
{
...
void MctSolverIlpCallbackUser::main()
{
...
//    out.close();
    ...
    if (g_tol.less(_minCutAlg.minCutValue(), 1.))
    {
      ...
      add(1 <= sum, IloCplex::UseCutPurge).end();
    }
  }
